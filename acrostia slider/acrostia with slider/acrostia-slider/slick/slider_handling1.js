...
$('.add-slide').on('click', function () {
  ...
  set_img_pre = '<div id="output' + localStorage.length + '" class="js-remove-slide" onClick=deleteItem(this)> <img src="images/' + name + '"/> <p id="imgname">' + name + '</p> <i  class="fa fa-trash " aria-hidden="true" ></i><hr></div>';
  ...
  if(radioValue == "parallel_text"){
    ...
    var img = '<div id="output'+localStorage.length+'" class="img-text"><h1 class="text-slider">' + parallel_text + '</h1><div class="imges"><img src="images/' + name + '"/></div>';
    slideIndex++;
    ...
    if (extension.match(/png|jpeg|jpg/)) {
      // if (center_text.val() != null && center_text.val() != '') {
        ...
        var img = '<div id="output'+localStorage.length+'" class="img-item"><div class="text"><P id="p' + i + '">' + center_text + '</P></div><img src="images/' + name + '"/></div>';
      }
      else {
        var img = '<div id="output'+localStorage.length+'" class="img-item "><div class="text"><P id="p' + i + '"></P></div><img src="images/' + name + '"/></div>';
      }
      i++;
      slideIndex++;
      ...
      function videofunction() {
        ...
        var video = '<div id="output'+localStorage.length+'" class="img-item"><iframe src="images/' + video_name + '"/></iframe></div>';
        slideIndex++;
...
function deleteItem(e){
    ...
    for(var i=localStorage.length;i>=0;i--){
      var name=localStorage.getItem('storageadd'+i);

      if(name.includes(id)){
        localStorage.removeItem('storageadd'+i);
         sremove();
         alert('deleted');
      }
...
function getId(e) {
  ...
  //  var newtab = window.open('file:///D:/INEXTURE/acrostia%20with%20slider/acrostia-slider/indexf.html');
